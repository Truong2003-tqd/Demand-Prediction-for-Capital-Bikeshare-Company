...
bike_sharing_data %>% 
  ...
  mutate(Proportion = Total*100/sum(Total)) %>%
  ungroup() %>%    
  ggplot(aes(x = factor(hr_cat), y = Proportion, fill = UserType))+
  geom_col(position = position_dodge(), width = 0.8)+
  scale_x_discrete(breaks = c(0,6,12,18,23))+
  scale_fill_manual(values = c("Casual" = "#1f77b4", "Registered" = "#ff7f0e")) +
  labs(
    title = "Proportion of Hourly Usage across 2011 and 2012",
    x = "Hour Blocks", y = "Proportion of Total Users", fill = "User Type"
  ) +
...
bike_sharing_data %>%
  select(hum, casual, registered) %>%
  pivot_longer(cols = c(casual, registered), names_to = "UserType", values_to = "Usage") %>%    
  ggplot(aes(x = hum, y = Usage, color = UserType)) +
  geom_point(size = 1, alpha = 0.5) +
  geom_smooth(method = "lm", color = "black")+
  scale_color_manual(values = c("casual" = "#1f77b4", "registered" = "#ff7f0e")) +
  facet_grid(~UserType) +
  labs(
    title = "Scatterplot of Humidity vs Usage",
    x = "Humidity",
    y = "Usage"
  ) +
  theme_classic()
...
bike_sharing_data %>% 
  group_by(season, hr_cat) %>%
  reframe(Casual = casual) %>%
  pivot_longer(cols = c(Casual),
               names_to = "UserType",
               values_to = "Usage") %>%
  ...
  labs(
    title = "Box Plots of Casual Users Across Hour Blocks in Each Season",
    x = "Season", y = "Usage", fill = "User Type") +
  facet_grid(~season)+
  theme_classic()
...
bike_sharing_data %>% 
  ...
  pivot_longer(cols = c(Registered),
               names_to = "UserType",
               values_to = "Usage") %>%
  ggplot(aes(x = hr_cat, y = Usage, fill = UserType))+
  geom_boxplot()+
...
bike_sharing_data %>% 
  group_by(season,weathersit, hr_cat) %>%
  reframe(Registered = registered) %>%
  pivot_longer(cols = c(Registered),
               names_to = "UserType",
               values_to = "Usage") %>%
